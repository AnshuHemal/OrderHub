...
export default function CustomersPage() {
  ...
  const inputCls = (err: string) =>
    `w-full p-2.5 bg-stone-50 dark:bg-stone-955 border rounded-xl text-stone-800 dark:text-stone-200 ${
  ...
  return (
    <div className="space-y-6 animate-fade-in text-xs">
      <div className="flex justify-between items-center pb-4 border-b border-stone-250 dark:border-stone-855">
        ...
        {isAdmin && (
          <button
            onClick={() => { resetForm(); setEditingCustomerId(null); setShowCustomerForm(true); }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
          <div className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-850 p-6 rounded-3xl shadow-xl w-full max-w-md">
            ...
            <form onSubmit={handleCustomerSubmit} className="space-y-4">
              <div>
                <label className="block text-stone-500 dark:text-stone-400 font-bold mb-1">
                  Full Name <span className="text-red-500">*</span>
                </label>
                <input
                  type="text" value={custName}
              ...
              <div>
                <label className="block text-stone-550 dark:text-stone-400 font-bold mb-1">
                  Email Address <span className="text-red-500">*</span>
                </label>
                <input
                  type="text" value={custEmail}
              ...
              <div>
                ...
                <input
                  type="text" value={custPhone}
                  onChange={(e) => { setCustPhone(e.target.value); setPhoneErr(""); }}
                  placeholder="+1 555-0199"
                  className={inputCls(phoneErr)}
                />
                {phoneErr && <p className="text-red-500 text-[11px] mt-0.5">{phoneErr}</p>}
              </div>

              <div className="flex justify-end gap-2 pt-2">
                <button
                  type="button"
                  onClick={() => { setShowCustomerForm(false); resetForm(); }}
                    ...
                    <td className="px-6 py-4 text-right">
                      {isAdmin ? (
                        <div className="inline-flex gap-2">
                          ...
                          <button
                            onClick={async () => {
                              if (confirm(`Delete customer ${cust.name}?`)) await deleteCustomer(cust.id);
                            }}
