...
export default function AdminBackend() {
  ...
  // Auth check - allow view but display alert if employee tries to edit (for testing, we can let both edit but warn)
  ...
  const handleProductSubmit = (e: React.FormEvent) => {
    ...
    const catIdNum = prodCatId ? parseInt(prodCatId) : null;
  ...
  const startEditProduct = (prod: Product) => {
    ...
    setProdCatId(prod.categoryId ? String(prod.categoryId) : "");
  ...
  const handlePromoSubmit = (e: React.FormEvent) => {
    ...
    createPromotion({
      name: promoName,
      promoType: promoType,
      targetProductId: promoType === "product" ? parseInt(promoTargetProduct) : null,
  ...
  const handleBookingSubmit = (e: React.FormEvent) => {
    ...
    createBooking({
      customerId: parseInt(bookingCustId),
      tableId: parseInt(bookingTableId),
      bookingTime,
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    // 2. Employee filter
    if (filterEmployeeId !== "all" && order.employeeId !== parseInt(filterEmployeeId)) {
      return false;
    }
    ...
    if (filterProductId !== "all") {
      const hasProduct = order.items.some(it => it.productId === parseInt(filterProductId));
  ...
  const productSalesMap: Record<number, { name: string; qty: number; rev: number }> = {};
  paidOrders.forEach((order) => {
    order.items.forEach((item) => {
      // If product filter is specific, ignore other products in the list
      if (filterProductId !== "all" && item.productId !== parseInt(filterProductId)) return;
  ...
  const categorySalesMap: Record<number, { name: string; color: string; rev: number }> = {};
  paidOrders.forEach((order) => {
    order.items.forEach((item) => {
